...
header('Content-Type: text/html; charset=ISO-8859-1');
    ...
    $result = $mysqli->query("select docentes.iddocente,dia,hora_inicio,hora_fin,codcurso from docentes inner join horarios on horarios.iddocente=docentes.iddocente and docentes.correo='".$correo."' and ciclo='2020-2'");
   ...
   // var_dump($nombre_curso);
	...
	$result = $mysqli->query("select docentes.horas_min,docentes.horas_max,codcurso,uactualizacion,time(sum(horarios.hora_fin-horarios.hora_inicio)) as 
		horas_dictadas from docentes inner join horarios on
 		horarios.iddocente=docentes.iddocente and docentes.correo='".$correo."' and ciclo='2020-2'");
	if ($result->num_rows > 0) {
        	$row = $result->fetch_assoc();
...
<div class="container" id="ver_horario_2" style="display: none;" >
	<h2>Horario propuesto</h2>
